...
export default async function InventoryPage() {
  ...
  const [profileRes, posRes, deptRes] = await Promise.all([
    ...
    supabase.from('purchase_orders').select('*, documents:po_documents(*)').order('created_at', { ascending: false }),
  ...
  return (
    <div className="flex h-screen overflow-hidden">
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden">
        <TopBar title="Inventory" departmentName={profile.department?.name ?? 'Ole Miss Athletics'} roleLabel={profile.role.replace('_', ' ')} />
        <InventoryClient pos={allPOs} departments={depts} profile={profile} />
      </main>
    </div>
  )
}
